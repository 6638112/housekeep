...
import com.connxun.app.repositories.LwUserRepositoryCustom;
 ...
 */
@Repository
public class LwUserRepositoryImpl extends JdbcDaoSupport implements LwUserRepositoryCustom {

    @Autowired
    public LwUserRepositoryImpl(DataSource dataSource) {
    ...
    public List<LwUser> getList(LwUserSearchVO searchVO) {
        ...
        String sql = "SELECT  *  FROM lw_user where 1=1  ";
        ...
        if (count > 0) {
            ...
            return jdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(LwUser.class));//执行sql 返回数据
        }
    ...
    public LwUser getPhone(String phone) {
        String sql = "SELECT * FROM  lw_user WHERE  phone =" + phone;
        List<LwUser> list = this.getJdbcTemplate().query(sql, new BeanPropertyRowMapper<>(LwUser.class));
        if (list.size() > 0)
            return list.get(0);
        else
            return null;
    ...
    @Override
    public List<LwUser> search(LwUserSearchVO searchVO) {
    ...
    public List<LwUser> getInvitationList(LwUserSearchVO lwUserSearchVO) {
        ...
        String sql = "SELECT phone,createDate ,icon FROM  lw_user  WHERE  1=1   ";
        sql += createSearchSql(lwUserSearchVO);//调用创建查询sql方法
        sql += " order by createDate DESC ";//拼接sql 排序
        SqlParameterSource params = new BeanPropertySqlParameterSource(lwUserSearchVO);//实例化一个查询类，用来注入查询数据 根据（:+字段值）
        NamedParameterJdbcTemplate jdbcTemplate = new NamedParameterJdbcTemplate(getJdbcTemplate());//对象构造器初始化
        String countSql = PagerUtils.count(sql, JdbcConstants.MYSQL);//工具类自动创建查询总数sql
        int count = jdbcTemplate.queryForObject(countSql, params, Integer.class);//执行sql返回查询结果
        if (count > 0) {
            lwUserSearchVO.setTotal(count);//set一个总数到查询VO的父类
            return jdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(LwUser.class));//执行sql 返回数据
        }
        return new ArrayList<>();
    ...
    public LwUser isInvite(Integer inviteCode) {
        String sql = "SELECT * FROM  lw_user WHERE  inviteCode =" + inviteCode;
        List<LwUser> list = this.getJdbcTemplate().query(sql, new BeanPropertyRowMapper<>(LwUser.class));
        if (list.size() > 0)
            return list.get(0);
        else
     ...
     * @return 查询sql
     */
    private String createSearchSql(LwUserSearchVO searchVO) {
        String sql = "";
