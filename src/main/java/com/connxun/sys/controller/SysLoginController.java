...
import com.connxun.sys.service.SysLoginService;
...
import com.connxun.util.verifycode.VerifyCodeUtil;
...
public class SysLoginController {
    ...
    public void checkLogin(HttpServletRequest request, HttpServletResponse response, String username, String password, String verifyCode) {
        ...
        if (sysUser == null || !sysUserService.checkPass(sysUser, password)) {
            ...
            boolean isNeedVerify = userLoginService.isNeedVerifyCode(username);
            String json = "{\"success\":" + false + ",\"msgText\":\"用户名或密码错误\",\"isNeedVerifyCode\":" + isNeedVerify + "}";
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/isNeedVerifyCode.do")
    public void isNeedVerifyCode(HttpServletRequest request, HttpServletResponse response, String username) throws Exception {
        boolean flag = userLoginService.isNeedVerifyCode(username);
        WebUtil.out(response, "{\"success\":" + flag + "}");
    }
