...
!function ($) {
    ...
    var Tag = function (element, options) {
        ...
        this.element.removeAttr('name');//移除该元素的name，将name放到span option中
    ...
    Tag.prototype = {
        ...
        , show: function () {
            ...
            that.element
                .wrap($('<div class="tags" id="tag-input">'))
                .parent()
                .on('click', function () {
                    that.input.focus()
                })
            ...
            that.input = $('<input type="text" id="search-input">')
                ...
                .on('keydown', function (event) {
                    if (!$.trim($(this).val()) && event.keyCode == 8) {
        ...
        , createBadge: function (value, id) {
            var that = this

            $('<span/>', {
                'class': "tag topic_item",
                'data-id': id
            }).text(value.toString())
