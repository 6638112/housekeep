...
<head>
    <title>${webTitle }-
        <c:if test="${type==0}">课程科目</c:if>
        <c:if test="${type==1}">考区管理</c:if>
            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                                ...
                                <div class="widget-main no-padding">
                                    <form:form action="/" method="post"
                                        ...
                                        <div class="row">
                                         ...
                                         <%--<div class="col-xs-3">--%>
                                                <%--<div class="form-group">--%>
                                                    <%--<label class="control-label col-xs-6 col-lg-5">课程金额：</label>--%>
                                                    <%--<div class="col-xs-6 col-lg-7">--%>
                                                        <%--<input type="text" id="moneySearch"--%>
                                                               <%--class="col-xs-12 col-sm-12 col-lg-10"--%>
                                                               <%--value="" title=""/>--%>
                                                    <%--</div>--%>
                                                <%--</div>--%>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table id="lwSubject" class="table table-striped table-bordered table-hover">
                                    <thead>
                                    <tr>
                                        <th style="background-color: #E5E5E5">    <c:if test="${type==0}">课程科目</c:if>
                                            <c:if test="${type==1}">考区名称</c:if>
                                        </th>
                                        <th style="background-color: #E5E5E5">排序</th>
                                        <th style="background-color: #E5E5E5">创建时间表</th>
                                        <th style="background-color: #E5E5E5">创建人员</th>
    ...
    <div class="modal-dialog" style="width: 50%;top: 18%">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-xs-12">
                        <form id="lwSubjectForm" name="lwSubjectForm" class="form-horizontal"
                              action="${dynamicServer}/cms/subject/save.do" method="post">
                            <input id="id" name="id" value="" type="hidden" title=""/>

                            <div class="form-group">
                                <label class="col-sm-3 control-label">    <c:if test="${type==0}">课程科目</c:if>
                                    <c:if test="${type==1}">考区管理</c:if>：</label>
                                <input type="hidden" name="type" value="${type}">
                                <div class="col-sm-9">
                                    <input type="text" id="subject" name="subject" maxlength="256" class="col-xs-10 col-sm-7"
                                           placeholder=""
...
<script>
    $(function () {
        /*把要初始化的table的对象赋值*/
        var $table_id = $("#lwSubject");
        /*自定义查询参数*/
        var userParam = {
            getQueryCondition: function (data) {
                var param = {};
                param.subject = $("#nameSearch").val();
        ...
        window.table = $table_id.DataTable({
            ...
            ajax: function (data, callback) {//ajax配置为function,手动调用异步查询
                $.ajax({
                    type: "GET",
                    url: '${dynamicServer}/cms/subject/getList.do?type=${type}',//请求数据的参数
    ...
    /*新增时清空表单数据*/
    function flushForm() {
        $("#lwSubjectForm").find("input,select").each(function () {
            var id = $(this).attr("id");
            $("#" + id).val("");
        });
        $.ajax({
            type: "get",
            url: "${dynamicServer}/cms/subject/findAll.do",
    ...
    function updateDepartment(id) {
        ...
        $.ajax({
            type: "GET",
            url: "${dynamicServer}/cms/subject/findOne.do",
            data: {
                "id": id
            },
            contentType: 'application/json',
    ...
    function delDepartment(id) {
        bootbox.confirm("<a style='font-size: 17px;color: red'>确定要删除该信息？</a>", function (result) {
            if (result) {
                $.ajax({
                    type: "GET",
                    url: "${dynamicServer}/cms/subject/delete.do",
                    data: {
                        "id": id
                    },
                    contentType: 'application/json',
